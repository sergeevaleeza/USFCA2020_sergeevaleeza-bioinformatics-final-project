...
state_death_chosen_date_ordered <- states_data_chosen_date[
...
state_death_chosen_date_five <- state_death_chosen_date_ordered %>% top_n(5)

# create list to test if Texas is included in the data frame
list_state_death_chosen_date_five = as.list(
  state_death_chosen_date_five$state)

# if TX is not part of the selected states, add TX data to data frame
if ("TX" %in% list_state_death_chosen_date_five){
  state_death_chosen_date_five_TX <- state_death_chosen_date_five
  } else {
  df_tx <- subset(state_death_chosen_date_ordered, state == "TX")
  state_death_chosen_date_five_TX <- rbind(state_death_chosen_date_five,df_tx)
  }

# this list is used in the graphs related to population density
list_state_death_chosen_date_five_TX <- 
  as.list(state_death_chosen_date_five_TX$state)

# create bar plot showing 5 US states and the number of people who died of 
# COVID-19 or COVID-19 related issues
top_state_death_plot <- ggplot(data = state_death_chosen_date_five_TX, 
...
``` {r total-hospital-plot, fig.height=12}
...
state_hospital_chosen_date_ordered <- states_data_chosen_date[
...
state_hospital_chosen_date_five <- state_hospital_chosen_date_ordered %>%
  top_n(5)

# create list to test if Texas is included in the data frame
list_state_hospital_chosen_date_five = as.list(
  state_hospital_chosen_date_five$state)

# if TX is not part of the selected states, add TX data to data frame
if ("TX" %in% list_state_hospital_chosen_date_five){
  state_hospital_chosen_date_five_TX <- state_hospital_chosen_date_five
  } else {
  df_tx <- subset(state_hospital_chosen_date_ordered, state == "TX")
  state_hospital_chosen_date_five_TX <- rbind(state_hospital_chosen_date_five,df_tx)
  }

# create bar plot showing 10 US states and number of people who died of COVID-19
# or COVID-19 related issues
top_state_hospital_plot <- ggplot(data = state_hospital_chosen_date_five_TX, 
...
``` {r total-positive-plot, fig.height=12}
...
state_positive_chosen_date_ordered <- states_data_chosen_date[
  order(states_data_chosen_date$positive, decreasing=TRUE), ]
  
# take top 5 states with highest total hospitalizations
state_positive_chosen_date_five <- state_positive_chosen_date_ordered %>%
  top_n(5)

# create list to test if Texas is included in the data frame
list_state_positive_chosen_date_five = as.list(
  state_positive_chosen_date_five$state)

# if TX is not part of the selected states, add TX data to data frame
if ("TX" %in% list_state_positive_chosen_date_five){
  state_positive_chosen_date_five_TX <- state_positive_chosen_date_five
} else {
  df_tx <- subset(state_positive_chosen_date_ordered, state == "TX")
  state_positive_chosen_date_five_TX <- rbind(
    state_positive_chosen_date_five,df_tx)
  return(state_positive_chosen_date_five_TX)
  }

# select row with Texas data, and add it to the top 5 states data frame
#df_tx <- subset(state_positive_chosen_date_ordered, state == "TX")
#state_positive_chosen_date_five_TX <- rbind(
#  state_positive_chosen_date_five,df_tx)
...
top_state_positive_plot <- ggplot(data = state_positive_chosen_date_five_TX,
                                  aes(x = positive,
...
``` {r increase-death-positive-test-plots}
# take previously 
all_state_data <- all_state_covid_data()

# select all rows matching the list of five states with highest deaths
fiveTX_state_death_increase <- all_state_data %>%
  select(date, state, deathIncrease, positiveIncrease) %>%
  group_by(state) %>%
  dplyr::filter(`state` == list_state_death_chosen_date_five_TX)

# convert date to format YYYY-MM-DD
fiveTX_state_death_increase$date <- 
  lubridate::ymd(fiveTX_state_death_increase$date)

# first plot of time series of death increase for top 5 states from 
# previous graph of top moralities on given date
t_plot <- ggplot(data = fiveTX_state_death_increase, 
                 mapping = aes(x = date, y = deathIncrease, color = state)) +
  geom_line() +
  facet_grid(rows = vars(state)) +
  labs(x = "Date", y = "Death Increase") +
  scale_y_continuous(breaks=c(0, 300, 600, 1000)) +
  theme_bw()

# second plot of time series of positive tests increase for top 5 states from 
# previous graph of top moralities on given date
p_plot <- ggplot(data = fiveTX_state_death_increase, 
                 mapping = aes(x = date, y = positiveIncrease, color = state)) +
  geom_line() +
  facet_grid(rows = vars(state)) +
  labs(x = "Date", y = "Positive Test Increase") +
  scale_y_continuous(breaks=c(0, 7000, 14000)) +
  theme_bw()

two_timeseries_plot_deathIncrease_positiveIncrease <- 
  t_plot / p_plot + plot_layout(heights = c(10, 10))

ggsave(plot = two_timeseries_plot_deathIncrease_positiveIncrease,
...
``` {r population-and-density-plot}
# load population data
population_density_data <- readr::read_csv(
    "data/raw_data/2020_population_density_US.csv")

# select top 5 states with highest total deaths at given date
population_density_top_five_states <- population_density_data %>%
  select(State, Density_p_mi2, Pop_2020, LandArea_mi2) %>%
  dplyr::filter(`State` %in% list_state_death_chosen_date_five_TX)

# bar chart to show population density
population_density_top_five_states_plot <- ggplot(
  data = population_density_top_five_states,
...
# save graph into data/images folder
#ggsave(plot = population_density_top_five_states_plot,
#       filename = paste0("data/images/population_density_
#                         top_five_states_plot.png"))


# bar chart to show population density
population_total_top_five_states_plot <- ggplot(
  data = population_density_top_five_states,
...
pop_total_vs_density_plots <-
  population_total_top_five_states_plot / 
  population_density_top_five_states_plot + plot_layout(heights = c(10, 10))
...
pop_total_vs_density_plots
```
**Figure 5**: Top graph shows total US population in 2020 for select 5 states that have high number of deaths up to select date. Bottom graph shows population density in 2020 of the related 5 states with the high number of deaths up to selected date.
...
# select top 5 states with highest total deaths at given date
population_density_top_states <- population_density_data %>%
  select(State, Density_p_mi2)

  population_density_top_states_ordered <- population_density_top_states[
    order(population_density_data$Density_p_mi2, decreasing=TRUE), ]

  population_density_top_states_ordered %>%
    top_n(15) %>%
